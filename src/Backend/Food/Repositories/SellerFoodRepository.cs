...
public class SellerFoodRepository : ISellerFoodRepository
{
    ...
    public async Task<Entities.SellerFood> SellerFood(int foodId, SellerFood sellerFood)
    {
        if (sellerFood.Count != 0)
        {
            ...
            if (findFood is not null)
            {
                ...
                {
                    if(sellerFood.Status == "correct")
                    {
                        checkSellerFood.Count += sellerFood.Count - (checkSellerFood.Count - 1);
    ...
    {
        var deleteFood = _context.SellerFoods.Find(id);
        if(deleteFood != null)
        {
            _context.SellerFoods.Remove(deleteFood);
            await _context.SaveChangesAsync();
        }
    }
